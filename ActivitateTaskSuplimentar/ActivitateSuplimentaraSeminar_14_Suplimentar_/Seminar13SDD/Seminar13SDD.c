...
//dezalocare
// Am implementat o propunere pentru dezalocarea nodurilor principale si secundare
// Dezalocare noduri secundare
void DezalocareNoduriSecundare(NodSecundar* capdenod) {
	while (capdenod) {
		NodSecundar* temporar = capdenod;
		capdenod = capdenod->next;
		free(temporar);
	}
}
// Dezalocare noduri principale
void DezalocareNoduriPrincipale(NodPrincipal* altcapdeNod) {
	while (altcapdeNod) {
		NodPrincipal* alttemp = altcapdeNod;
		altcapdeNod = altcapdeNod->next;
		free(alttemp->info.magistrala);
		DezalocareListaPrincipala(alttemp->vecini);
		free(alttemp);
	}
}
// Am pus dezalocarea secundara in detrimentul celei principale pentru nu a genera memory leak
void main() {
